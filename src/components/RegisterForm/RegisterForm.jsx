...
const RegisterForm = () => {
    ...
    const { register, handleSubmit ,formState: { errors: formErrors , isDirty , isValid  } } = useForm({ mode: "onChange" });
    ...
    const onSubmit = async data => {
        try {
            console.log({REGISTER_USER_API, data})
            const isValidPassword = validatePassword(data.password, data.retypePassword)
            if(!isValidPassword) throw new Error('passwords not matched')

            const {retypePassword, ...userData} = data

            const response = await axios.post(REGISTER_USER_API, {data: { ...userData, role: 'user' }});
    ...
    const validatePassword = (password, retypePassword) => password === retypePassword
    ...
    return (
        ...
        <Container  component="main" maxWidth="sm">
            ...
            <form
                ...
                {formErrors.email && <span className={classes.messageError}>{formErrors.email.message}</span>}
                <Typography className={classes.passwordTypo} variant="h6" gutterBottom component="div">
                    Password
                </Typography>
                <TextField
                    variant="standard"
                ...
                {formErrors.password && <span className={classes.messageError}>{formErrors.password.message}</span>}
                <TextField
                    variant="standard"
                    margin="normal"
                    fullWidth
                    ...
                    {...register("retypePassword", { required: "This field is required." })}
                ...
                {formErrors.retypePassword && <p className={classes.messageError}>{formErrors.retypePassword.message}</p>}
                <Button 
                    type="submit"
                    fullWidth
                    variant="contained"
            ...
            <Typography className={classes.terms} variant="caption" display="block" gutterBottom>
                <u><b>Our Terms of Use and Privacy Policy</b></u>
            </Typography>
